...
exports.get3rdSemStudents = async (req, res, next) => {
  try {
    const sem = req.params.sem;
    const students = await Student.find({ semester: sem });

    const aDivision = students.filter((s) => s.division === "A");
    const bDivision = students.filter((s) => s.division === "B");
    const cDivision = students.filter((s) => s.division === "C");

    res
      .status(200)
      .json({ len: students.length, aDivision, bDivision, cDivision });
  } catch (e) {
    res.status(404).json({ msg: e.message });
  }
};
